...
function migrateActorSystemData(actorType, system) {
  ...
  if (actorType === "character") {
    // Ensure dread data is properly structured
    if (system.dread) {
      // If dread.value is not an array, convert it to an array
      if (!Array.isArray(system.dread.value)) {
        updateData["dread.value"] = [system.dread.value || 1];
      }
      
      // If dread.states is not an array, initialize it
      if (!Array.isArray(system.dread.states)) {
        updateData["dread.states"] = [true, false, false, false, false, false];
      }
      
      // Ensure min and max values are set
      if (system.dread.min === undefined) updateData["dread.min"] = 0;
      if (system.dread.max === undefined) updateData["dread.max"] = 6;
    }
  }
