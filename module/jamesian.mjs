...
import { jamesianActor } from "./documents/actor.mjs";
import { jamesianItem } from "./documents/item.mjs";
// Import sheet classes.
import { jamesianActorSheet } from "./sheets/actor-sheet.mjs";
import { jamesianItemSheet } from "./sheets/item-sheet.mjs";
// Import helper/utility classes and constants.
import { jamesian } from "./helpers/config.mjs";
import * as utils from "./helpers/utils.mjs";
// Import data models.
import { CharacterData, NPCData } from "./data/actor.mjs";
import { jamesianItemData } from "./data/item.mjs";
...
Hooks.once("init", async function () {
	...
	CONFIG.jamesian = jamesian;

	// Define custom Document classes
	CONFIG.Actor.documentClass = jamesianActor;
	CONFIG.Item.documentClass = jamesianItem;
	...
	CONFIG.Item.dataModels = {
		item: jamesianItemData,
	};

	// Register sheet application classes
	Actors.unregisterSheet("core", ActorSheet);
	Actors.registerSheet("jamesian", jamesianActorSheet, {
		makeDefault: true,
		label: "jamesian.SheetLabels.Investigator",
	});
	Items.unregisterSheet("core", ItemSheet);
	Items.registerSheet("jamesian", jamesianItemSheet, {
		makeDefault: true,
		label: "jamesian.SheetLabels.Gear",
	});
...
Hooks.on("renderChatMessage", (chatMessage, [html], messageData) => {
	const flag = chatMessage.getFlag("jamesian", "chatID");
	if (flag && flag == "jamesian") {
...
Hooks.on("renderSettings", (app, html) => {
	...
	const MODULE_CONFIG = {
		headingKey: "jamesian.Settings.game.heading",
		...
		buttonsData: [
			{
				...
				labelKey: "jamesian.Settings.game.publisher.title",
			},
			{
				action: (ev) => {
					ev.preventDefault();
					window.open("https://github.com/philote/jamesian", "_blank");
				},
				iconClasses: ["fab", "fa-github"],
				labelKey: "jamesian.Settings.game.github.title",
			},
			{
				action: (ev) => {
					ev.preventDefault();
				...
				labelKey: "jamesian.Settings.game.kofi.title",
			},
		],
	};
